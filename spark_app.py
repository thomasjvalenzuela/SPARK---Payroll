...
# Assets are read-only repo files on Streamlit Cloud (do NOT mkdir/write here)
ASSETS_DIR = BASE_DIR / "assets"
REPO_LOGO_PATH = ASSETS_DIR / "appliancelogodefault.png"
LOGO_URL = ""  # optional fallback URL (must be non-empty to use)

# Writable data directory (Streamlit Cloud safe)
...
# If you want to allow downloading a logo when assets file isn't present, store it here:
...
def _get_best_logo_path() -> Path | None:
    """Return a local Path to a logo if available, else None."""
...
def download_logo_if_needed():
    """
    If repo logo isn't present and LOGO_URL is set, download to writable runtime path.
    Safe for Streamlit Cloud because it writes to /tmp (via STORAGE_DIR).
    """
    if (REPO_LOGO_PATH.exists() and REPO_LOGO_PATH.is_file()) or (RUNTIME_LOGO_PATH.exists() and RUNTIME_LOGO_PATH.is_file()):
        return
...
def render_logo(width: int = 200):
    """
    Render a logo safely:
    - Prefer local repo file
    - Fallback to downloaded runtime logo
    - Fallback to URL only if non-empty
...
def detect_tech_from_df_and_filename(df: pd.DataFrame, filename: str) -> str | None:
    """
    Try to find a tech identifier (initials or name) from:
    - common tech columns
    - any column containing 'tech'/'technician'
    - filename (e.g. 'MG_commissions.csv' -> 'MG')
    """
...
def parse_job_csv(file, week_start_dt):
    ...
    Robust SD commission parser:
      ...
      - PayCode ∈ {1,2} (only if present)
      - $ → numeric
    Returns: (tech_identifier, cleaned_df)
    """
    try:
        ...
        raw["__Date"] = raw[date_col].apply(_parse_possible_date) if date_col else pd.NaT
        ...
        tt_col = _find_col(cols, ["TotalCmssn", "TotalCommission", "Commission_Total"])
        ...
        for col, newname in [
            ...
            df[newname] = _to_money_series(df[col]) if col else 0.0

        if df["TotalCmssn"].sum() == 0 and any(df[c].sum() for c in ["MrchCmssn", "PartsCmssn", "ScallCmssn", "LbrCmmsn"]):
            df["TotalCmssn"] = df[["MrchCmssn", "PartsCmssn", "ScallCmssn", "LbrCmmsn"]].sum(axis=1) if "LbrCmmsn" in df.columns else df[["MrchCmssn", "PartsCmssn", "ScallCmssn", "LbrCmmsn"]].sum(axis=1)
...
def parse_tips_csv(file):
    ...
    Returns columns: ['Employee','Tip','NormName','Initials'].
    """
    ...
    header_columns = [col.strip().strip('"') for col in lines[header_line_idx].split(",")]
    ...
    for i in range(data_start_idx, len(lines)):
        ...
        try:
            ...
            if tip_amount > 0:
                tips_data.append({"Employee": employee_name, "Tip": tip_amount})
...
def match_tips_to_tech(tips_df: pd.DataFrame, tech_name: str, tech_initials: str) -> float:
    ...
    NICK_MAP = {"john": "john", "jon": "john", "johnny": "john", "doe": "doe"}
    ...
    if canon_full:
        ...
        contains_sum = tips_df.loc[tips_df["NormName"].str.contains(pattern, regex=True, na=False), "Tip"].sum()
        if contains_sum:
...
def create_paystub_pdf(
    ...
    # Logo (local if available)
    logo_path = _get_best_logo_path()
    if logo_path:
        try:
    ...
    disclaimer = Paragraph(
        ...
        "PTO, hourly pay, and time off details are available on your payroll provider pay stub.</i>",
        ParagraphStyle("Disclaimer", parent=styles["Normal"], fontSize=8, textColor=colors.HexColor(COLORS["metal_dark"]), alignment=TA_LEFT),
    )
                ...
                with st.expander(f"⚙️ {tech_id} - {data['file']}", expanded=False):
                    ...
                    with col1:
                        full_name = st.text_input(
                            "Full Name",
                            value=settings.get(tech_id, {}).get("full_name", ""),
                            key=f"name_{tech_id}",
                    ...
                    with col2:
                        branch = st.selectbox(
                            ...
                            index=0 if settings.get(tech_id, {}).get("branch") == "Branch A" else 1,
                    ...
                    with col4:
                        active = st.checkbox("Active", value=settings.get(tech_id, {}).get("active", True), key=f"active_{tech_id}")
    ...
    with tab3:
        ...
        if "detected_techs" in st.session_state:
            ...
            with st.form("adjustment_form"):
                ...
                with c1:
                    adj_tech = st.selectbox("Tech (initials/name)", options=tech_list)
                with c2:
                    adj_job = st.text_input("Job #", value="0")
                with c3:
                    adj_type = st.selectbox("Type", options=["Addition", "Deduction"])
                with c4:
                    adj_reason = st.text_input("Reason")
                with c5:
                    adj_amount = st.number_input("Amount", min_value=0.0, step=0.01)

                if st.form_submit_button("➕ Add Adjustment") and adj_amount > 0:
                    st.session_state.adjustments.append(
                        {"tech": adj_tech, "job": adj_job, "type": adj_type, "reason": adj_reason, "amount": adj_amount}
                    )
        ...
        if "detected_techs" not in st.session_state:
                    ...
                    if df is not None and not df.empty:
                        if "__Date" in df.columns:
                            try:
                                ...
                                st.write("Dates:", str(mind.date()) if pd.notna(mind) else "n/a", "→", str(maxd.date()) if pd.notna(maxd) else "n/a")
                            except Exception:
                                pass
                        if "PayCode" in df.columns:
            ...
            for tech_id, data in detected.items():
                if settings.get(tech_id, {}).get("active", True):
                    ...
                    branch = tech_entry.get("branch", "Branch A")
            ...
            if summary_rows:
                ...
                st.markdown(
                    f"**TOTALS:** Commission: ${s_df['Commission'].sum():,.2f} | Tips: ${s_df['Tips'].sum():,.2f} | Net Pay: ${s_df['Net Pay'].sum():,.2f}"
                )
                ...
                with st.spinner("Generating paystubs..."):
                    ...
                    for idx, tech_id in enumerate(active_techs, start=1):
                        ...
                        pdf_buffer = create_paystub_pdf(
                            ...
                            deductions=deductions,
                            net_pay=net_pay,
                            jobs_df=detected[tech_id]["jobs_df"],
                            adjustments_list=tech_adjustments,
                        )

                        safe_name = re.sub(r"[^A-Za-z0-9_-]+", "_", tech_name)
                        pdf_filename = f"paystub_{safe_name}_{week_str}.pdf"

                        # Write to /tmp-based output folder (safe)
                        (output_folder / pdf_filename).write_bytes(pdf_buffer.read())
